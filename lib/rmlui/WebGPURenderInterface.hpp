...
// Switch port feature toggles: keep the rendering features enabled on desktop while leaving the
// switch a single place to disable the expensive paths it can't support.
